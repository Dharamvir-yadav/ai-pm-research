...
name: create-stories
description: |
  Takes over from publish_confluence (if the user requested Confluence) or from feature_researcher (if the user skipped Confluence). Creates user stories in Gherkin format from the requirement set (Confluence page or signed-off research); ensures every story maps to hierarchical requirement IDs (REQ-AREA-NN, e.g. REQ-AUTH-01); runs traceability check. Does not publish to Jira—hands over to Devil's Advocate for review; after DA sign-off, flow moves to publish_jira.
...
- **Requirement mapping**: Each story must **map to at least one requirement ID**. In the story description (or acceptance criteria), include: **Maps to:** `REQ-AUTH-01` (and additional IDs if applicable). Follow `.cursor/rules/06-story-requirement-traceability.mdc`.
...
- Run the traceability check before handover:
  - Every requirement (`REQ-{AREA}-{NN}`) has **at least one story**.
  - Every story has **at least one requirement ID** (Maps to: `REQ-{AREA}-{NN}`).
...
| Step | Action |
|------|--------|
| 1 | Receive requirements: Confluence requirement page (if published) or handover from **feature_researcher** (if Confluence was skipped); in both cases requirements use hierarchical IDs `REQ-{AREA}-{NN}`. Or if none provided, ask user to provide link to confluence or copy requirements in chat |
| 2 | Create user stories in Gherkin format; include "Maps to:" each requirement ID; add NFRs when applicable. |
| 3 | Run traceability check (every requirement has ≥1 story, every story has ≥1 requirement ID). |
